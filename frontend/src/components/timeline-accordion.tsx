...
export function TimelineAccordion({ entries }: TimelineAccordionProps) {
  const groups = groupByWindow(entries);
      ...
      {groups.map((group, gi) => (
        <div key={gi} className="rounded-xl bg-white/5 p-3">
          <div className="mb-2 flex items-center justify-between">
            <div className="text-sm font-medium text-white/80">{group.label}</div>
            <div className="text-xs text-white/50">{group.items.length} events</div>
          </div>
          <div className="border-l border-white/10">
            {group.items.map((item, i) => (
              <TimelineRow key={`${gi}-${i}`} item={item} last={i === group.items.length - 1} />
            ))}
          </div>
        </div>
      ))}
...
function TimelineRow({ item, last }: { item: TimelineEntry; last: boolean }) {
  const [open, setOpen] = useState(true);
  return (
    <div className="relative pl-4">
      <div className="absolute left-[-6px] top-2 h-2 w-2 rounded-full bg-[#00A3E0]" />
      {!last && <div className="absolute left-[-1px] top-4 h-full w-px bg-white/10" />}
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <p className="pb-3 text-sm leading-relaxed text-white/70">{item.content}</p>
      </motion.div>
    </div>
  );
}
